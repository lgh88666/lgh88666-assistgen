...
AssistGen 的目标不是把功能堆满，而是围绕“多智能体电商导购”持续做深。后续会优先补强以下方向：

- **短期：提升导购效果**。扩充智能家居商品、场景、搭配关系和用户评价数据，让检索、推荐、解释链路有更充分的测试样本。
- **短期：完善质量评估**。补充“预算 1000 推荐 5000”“用户只想买低价却强行搭配”“售后投诉仍继续推销”等反例集，用于压测 Critic Agent。
- **中期：强化记忆系统**。完善 Redis 短期记忆、长会话压缩、`shopping_state` 持久化，以及不同 Agent 的选择性记忆注入。
- **中期：增强推荐推理**。继续打磨商品图关系推荐，让系统能稳定处理互补、替代、升级、套装、场景化搭配等真实导购逻辑。
- **中期：完善工程部署**。补齐 Docker Compose 版 Qdrant、Redis、Neo4j、后端和前端启动流程，降低复现实验成本。
- **长期：面向真实电商 Agent**。探索多轮需求澄清、用户偏好学习、商品反馈闭环、A/B 测试和推荐效果评估，让项目从 Demo 逐步接近可用的智能导购系统。
